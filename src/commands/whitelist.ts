...
import { WHITELIST_PATH } from "../../config.json";

const myWhitelist = new Whitelist(WHITELIST_PATH);
  ...
  execute: newSubcommandHandler([
    ...
    },
    {
      ...
      execute: async ({ interaction, args }) => {
        await interaction.deferReply({ ephemeral: true });
        const user = args.getUser("user", true);
        const result = await myWhitelist.remove(
    ...
    },
    {
      ...
      execute: async ({ client, interaction }) => {
        ...
        let accounts = "";
        let times = "";
        if (myWhitelist.data.length === 0)
          return interaction.editReply("There are no whitelist entries.");
        for (const user of myWhitelist.data) {
          users += `\n<@${user.discord_id}>`;
    ...
    },
    {
      ...
      execute: async ({ interaction, args }) => {
        await interaction.deferReply({ ephemeral: true });
        const user = args.getUser("user", true);
        const entries = await myWhitelist.filter(
          (entry) => entry.discord_id === user.id,
        );
        if (entries.length === 0)
          return interaction.editReply(
    ...
    },
    {
      ...
      execute: async ({ interaction, args }) => {
        await interaction.deferReply({ ephemeral: true });
        const username = args.getString("minecraft_username", true);
        const entry = await myWhitelist.filter(
          (entry) => entry.minecraft_username === username,
        );
        if (entry.length > 0) {
          await interaction.editReply(
